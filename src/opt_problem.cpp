...
SqpProblem::SqpProblem(int max_iter_, double conv_tol_, int num_dec_vars_, int num_eq_cons_, int num_ineq_cons_) : 
    max_iter(max_iter_),
    conv_tol(conv_tol_),
...
void SqpProblem::hessianLagrangian()
{
    ...
    double rho = 1.0/(delta_grad.transpose()*step + 1e-20);
...
{
    const bool bgfs_apprx = true;

    // hessian_lagrangian.setIdentity();
    ...
    {
        std::cout << "grad_lagrangian_norm: " << grad_lagrangian.norm() << std::endl;
        std::cout << "eq_cons_val: " << eq_cons_val.norm() << std::endl;
        std::cout << "ineq_cons_val: " << ineq_cons_val.norm() << std::endl;
